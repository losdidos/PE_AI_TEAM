...
for csv_path in sorted(DATA_DIR.glob("CLEAN_House*_15min_interpolated_with_metadata.csv")):
    ...
    for window in [4, 12, 24, 96]:
        ...
        df[f"roll_{window}_mean"] = shifted.rolling(window).mean()
        df[f"roll_{window}_std"] = shifted.rolling(window).std()
        df[f"roll_{window}_max"] = shifted.rolling(window).max()
        df[f"roll_{window}_min"] = shifted.rolling(window).min()
    ...
    for window in [4, 12, 24, 96]:
        features.extend([
            f"roll_{window}_mean", f"roll_{window}_std",
            f"roll_{window}_max", f"roll_{window}_min"
        ])
    ...
    plt.show()
    # Save the model
    #dump(model, MODELS_DIR / f"xgboost_model_house_{house_id:02d}.joblib")
